-- First, let's check what user table exists in your database
-- You may need to adjust the foreign key reference based on your actual table name

-- Option 1: If you're using Supabase auth.users table
-- CREATE TABLE IF NOT EXISTS activity_logs (
--     id BIGSERIAL PRIMARY KEY,
--     user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL,
--     ...

-- Option 2: If you have a custom users table with different name
-- First check your actual table name by running: \dt to see all tables

-- Create activity_logs table for storing user activity logs
...
CREATE TABLE IF NOT EXISTS activity_logs (
    ...
    record_id UUID, -- Changed to UUID to accommodate different table types
    ship_id UUID, -- Changed to UUID to match ships table
    ship_name VARCHAR(255),
    finding_id UUID, -- Changed to UUID to match findings table
    old_data JSONB,
    new_data JSONB,
...
-- Add foreign key constraints for ships and findings if tables exist
...
CREATE INDEX IF NOT EXISTS idx_activity_logs_ship_id ON activity_logs(ship_id);
CREATE INDEX IF NOT EXISTS idx_activity_logs_finding_id ON activity_logs(finding_id);
...
CREATE INDEX IF NOT EXISTS idx_activity_logs_ship_timestamp ON activity_logs(ship_id, timestamp DESC);
CREATE INDEX IF NOT EXISTS idx_activity_logs_user_activity ON activity_logs(user_id, activity_type, timestamp DESC);

-- Enable RLS (Row Level Security)
ALTER TABLE activity_logs ENABLE ROW LEVEL SECURITY;

-- Create policy for admin users to see all logs
-- This checks if the current user has admin role in auth.users metadata
...
-- Create policy for inserting logs (authenticated users can insert)
...
-- OPTIONAL: Add foreign key constraints after confirming table names
-- Uncomment and modify these after checking your actual table structure:

-- For Supabase auth.users:
-- ALTER TABLE activity_logs ADD CONSTRAINT fk_activity_logs_user_id 
--     FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE SET NULL;

-- For custom user table (replace 'your_user_table' with actual name):
-- ALTER TABLE activity_logs ADD CONSTRAINT fk_activity_logs_user_id 
--     FOREIGN KEY (user_id) REFERENCES your_user_table(id) ON DELETE SET NULL;
